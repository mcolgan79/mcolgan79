...
import { probAbove } from '../lib/probability'
import { pnlAtHorizon, pnlToday, singleExpiry } from '../lib/strategy'
...
const M = { top: 20, right: 104, bottom: 34, left: 60 }
const PLOT_H = 280
...
export function PayoffChart({ st, a }: Props) {
  const [wrapRef, width] = useMeasuredWidth<HTMLDivElement>()
  const [tableView, setTableView] = useState(false)
  const [hoverIdx, setHoverIdx] = useState<number | null>(null)

  const oneExpiry = singleExpiry(st)
  const horizonLabel = oneExpiry ? 'At expiration' : `At front expiry (${a.horizonDte}d)`
  const horizonShort = oneExpiry ? 'Expiration' : 'Front expiry'
  ...
  const data = useMemo(() => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      prices.push(s)
      expPnl.push(pnlAtHorizon(st, s))
      nowPnl.push(pnlToday(st, s))
    }
    return { prices, expPnl, nowPnl, lo, hi }
  }, [st, a])
          ...
          rows: [
            {
              ...
              label: horizonLabel.toLowerCase(),
            },
            {
              color: 'var(--series-2)',
            ...
            {
              value: fmtPct(1 - probAbove(data.prices[idx], gbm)),
              label: 'chance of finishing below',
            },
          ],
        }
  ...
  const emLeft = Math.max(st.S - a.expectedMove, data.lo)
  const emRight = Math.min(st.S + a.expectedMove, data.hi)
  ...
  return (
    <div className="card chart-card">
      ...
      {tableView ? (
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th scope="col">P&amp;L {horizonLabel.toLowerCase()}</th>
        ...
        <div className="chart-wrap" ref={wrapRef}>
          <svg
            ...
            role="img"
            aria-label="Profit and loss versus underlying price, at the front expiration and today"
          >
            <defs>
            ...
            <rect
              ...
              opacity={0.35}
            />
            <text x={x(st.S)} y={M.top - 7} textAnchor="middle" className="axis-text">
              ±1σ expected move
            </text>
            ...
            {/* profit / loss washes under the horizon curve */}
            ...
            {/* spot hairline */}
            <line x1={x(st.S)} x2={x(st.S)} y1={M.top} y2={M.top + PLOT_H} stroke="var(--baseline)" strokeWidth={1} />
            <text x={x(st.S) + 4} y={M.top + 12} className="marker-text">
              Spot {fmtNum(st.S)}
            </text>
            ...
            {breakevens.map((be) => (
              <g key={be}>
                <circle cx={x(be)} cy={y0} r={6} fill="var(--surface-1)" />
                <circle cx={x(be)} cy={y0} r={4} fill="var(--series-1)" />
                <text x={x(be)} y={y0 - 10} textAnchor="middle" className="marker-text">
                  BE {fmtNum(be)}
                </text>
              </g>
            ))}

            {/* strike ticks on the x-axis */}
            {strikes.map((k) => (
              <g key={k}>
            ...
            {/* axes */}
            {xTicks.map((v) =>
              strikes.some((k) => Math.abs(x(v) - x(k)) < 26) ? null : (
      ...
      <p className="chart-note">
        Green wash = profit {horizonLabel.toLowerCase()}, red = loss.
        {oneExpiry
          ? ' The T+0 curve marks the position to model (Black–Scholes) if the underlying moved right now.'
          : ' Longer-dated legs are marked to model at the front expiration, assuming their IVs hold.'}
      </p>
    </div>
  )
}
